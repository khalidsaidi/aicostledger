...
async function resolveIngestionToken(token: string) {
  const tokenHash = hashToken(token);
  const doc = await db.collection("ingestionTokens").doc(tokenHash).get();
  if (!doc.exists) {
    return null;
  }
  const data = doc.data() as IngestionTokenDoc | undefined;
  return data?.uid ?? null;
}

async function enforceRateLimit(uid: string, providerId: string) {
  const docRef = db.collection("users").doc(uid).collection("rateLimits").doc(providerId);
  const snapshot = await docRef.get();
  const now = Date.now();
  const last = snapshot.exists ? (snapshot.data()?.lastIngestAtMs as number | undefined) : undefined;
...
async function deleteReceipts(uid: string) {
...
app.post(
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    const existingHash = existing.exists ? (existing.data()?.hash as string | undefined) : undefined;
...
app.get(
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    let query = db.collection("users").doc(uid).collection("ledgerItems") as FirebaseFirestore.Query;
    if (providerId) {
    ...
    const items = snapshot.docs.map((doc) => doc.data() as LedgerItem);
    const bucket = storage.bucket();
...
app.get(
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    const item = doc.data() as LedgerItem;
    if (!item.pdfStoragePath) {
...
app.post(
  ...
  asyncHandler(async (req: AuthedRequest, res) => {
    ...
    const tokenHash = tokenDoc.exists ? (tokenDoc.data()?.hash as string | undefined) : undefined;
