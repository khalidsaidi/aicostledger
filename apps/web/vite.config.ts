...
export default defineConfig(({ mode }) => {
  ...
  const apiProxyTarget =
    env.VITE_API_PROXY_TARGET || env.VITE_DEV_PROXY_TARGET || fallbackTarget;
  const collectorProxyTarget =
    env.VITE_COLLECTOR_PROXY_TARGET || env.VITE_DEV_PROXY_TARGET || fallbackTarget;
